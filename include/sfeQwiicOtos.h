...
class sfeQwiicOtos
{
  public:
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getVersionInfo(sfe_otos_version_t &hwVersion, sfe_otos_version_t &fwVersion);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int calibrateImu(uint8_t numSamples = 255, bool waitUntilDone = true);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getImuCalibrationProgress(uint8_t &numSamples);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getLinearScalar(float &scalar);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getSignalProcessConfig(sfe_otos_signal_process_config_t &config);
    ...
    /// @param config Signal processing configuration
    /// @return 0 for succuss, negative for errors, positive for warnings
    int setSignalProcessConfig(sfe_otos_signal_process_config_t &config);
    ...
    int getPositionStdDev(sfe_otos_pose2d_t &pose);
    ...
    /// the covariance matrices of the Kalman filters used in the firmware, so
    /// they are just statistical quantities and do not represent actual error!
    int getVelocityStdDev(sfe_otos_pose2d_t &pose);
    ...
    /// the covariance matrices of the Kalman filters used in the firmware, so
    /// they are just statistical quantities and do not represent actual error!
    int getAccelerationStdDev(sfe_otos_pose2d_t &pose);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getPosVelAcc(sfe_otos_pose2d_t &pos, sfe_otos_pose2d_t &vel, sfe_otos_pose2d_t &acc);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getPosVelAccStdDev(sfe_otos_pose2d_t &pos, sfe_otos_pose2d_t &vel, sfe_otos_pose2d_t &acc);
    ...
    /// @return 0 for succuss, negative for errors, positive for warnings
    int getPosVelAccAndStdDev(sfe_otos_pose2d_t &pos, sfe_otos_pose2d_t &vel, sfe_otos_pose2d_t &acc,
  ...
  protected:
    ...
    int readPoseRegs(uint8_t reg, sfe_otos_pose2d_t &pose, float rawToXY, float rawToH);

    // Function to write raw pose registers and convert from specified units
    int writePoseRegs(uint8_t reg, sfe_otos_pose2d_t &pose, float xyToRaw, float hToRaw);
